...
raw_data <- read_csv("inputs/AAEdPQ3N.csv")
dict <- read_lines("inputs/gss_dict.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("inputs/gss_labels.txt")
